...
HG:RegisterTab("settings", "SETTINGS", 8, function(parent)
    ...
    HG:Button(section, "RESET WINDOW POSITION", 360, -246, 190, function()
        HG.db.position = nil
        HG.frame:ClearAllPoints()
        HG.frame:SetPoint("CENTER")
    end)
    HG:Button(section, "RESET ADDON SETTINGS", 14, -246, 190, function()
    ...
    HG:Button(community, "WOW HAVEN DISCORD", 80, -55, 190, function()
    ...
    local credit = HG:Label(about, "Addon vibecoded by Birb@.", 14, -48, 346)
    credit:ClearAllPoints()
    credit:SetPoint("CENTER", about, "CENTER", 0, -8)
